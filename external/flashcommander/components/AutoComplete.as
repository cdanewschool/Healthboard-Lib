package external.flashcommander.components
{
	...
	[Event (name="select", type="external.flashcommander.event.CustomEvent")]
	...
	public class AutoComplete extends SkinnableComponent
	{
		...
		override protected function partAdded(partName:String, instance:Object) : void{
			...
			if (instance==inputTxt){
				...
				inputTxt.text = _text;
				inputTxt.setFocus();	//added by DAMIAN
			}
			if (instance==list){
		...
		public function filterFunction(item:Object):Boolean{
			...
			// infix mode
				...
				if (label.search(text.toLowerCase()) != -1 || distance(label,text.toLowerCase()) <= 1) return true;		// || distance(label,text.toLowerCase()) <= 1	 (added by DB)
			}
			return false;
		}
		
		//DAMIAN -- taken from http://www.emanueleferonato.com/2010/06/09/find-levenshtein-distance-with-as3/
